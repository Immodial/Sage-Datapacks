...
# Show particles
particle small_gust ^ ^ ^
particle small_gust ^ ^ ^0.2
particle small_gust ^ ^ ^0.4
particle small_gust ^ ^ ^0.6
particle small_gust ^ ^ ^0.8
particle small_gust ^ ^ ^1
# Collide with blocks
...
execute positioned ~-0.5 ~-0.5 ~-0.5 as @n[type=!#iguns:ignored,dx=0,dy=0,dz=0] positioned ~0.5 ~0.5 ~0.5 run damage @s 10 iguns:bullet at ^ ^ ^-2
# Play whoosh pass by sound
execute positioned ~-2 ~-2 ~-2 run playsound entity.breeze.inhale player @a[dx=3,dy=3,dz=3] ~2 ~2 ~2 1 2
# Delete if too far
execute if score @s IGProjectileTime matches 80.. run kill @s