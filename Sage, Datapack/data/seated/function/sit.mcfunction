...
## >> Does: Sit down
## >> Input: None
execute align xyz positioned ~0.5 ~0.05 ~0.5 if entity @n[type=item_display,tag=STDSeat,distance=..0.55] run return fail
...
execute if block ~ ~ ~ #seated:half align xyz run tp @n[tag=STDSeating] ~0.5 ~0.55 ~0.5
execute if block ~ ~ ~ #minecraft:stairs[half=bottom] align xyz positioned ~0.5 ~0.55 ~0.5 run function seated:stairalign
