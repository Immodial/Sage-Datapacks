...
# Damage mobs
execute positioned ~-0.5 ~-0.5 ~-0.5 if entity @e[type=!#iguns:ignored,type=!enderman,dx=0,dy=0,dz=0] run scoreboard players add @s IGProjectileTime 5
execute positioned ~-0.5 ~-0.5 ~-0.5 as @e[type=!#iguns:ignored,dx=0,dy=0,dz=0] run damage @s 12 iguns:bullet at ^ ^ ^-2
# Play whoosh pass by sound
execute positioned ~-2 ~-2 ~-2 run playsound entity.breeze.inhale player @a[dx=3,dy=3,dz=3] ~2 ~2 ~2 1 2
# Delete if too far
execute if score @s IGProjectileTime matches 70.. run kill @s