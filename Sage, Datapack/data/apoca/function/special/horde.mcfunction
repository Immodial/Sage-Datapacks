...
execute as @e[tag=APOCZombieSpawner] at @s unless entity @e[type=#apoca:avoided,distance=..20] unless score _ZombiesPer APOCGlobals matches 80.. unless entity @e[type=#apoca:zombie,scores={APOCZombieDensity=12..},distance=..50] if predicate apoca:unlit run summon zombie ~ ~ ~
